########Differential gene expression analysis - Camponotus floridanus######
...
setwd("/Volumes/Pop_Gen/Differential_gene_expression/Species/Camponotus_floridanus/quants/")
...
head(txi.salmon.g$counts)[,1:3] #Adapt this section of code [,1:Nnumber of individuals being analysed]

# Transcript-level
head(txi.salmon.t$counts)[,1:3] #Adapt this section of code [,1:Nnumber of individuals being analysed]
...
#Select columns 1:7 - why? - I think this is the number of individuals being analysed
countData = txi.salmon.g$counts[,1:3]
...
id = c("SRR3726122", "SRR3726123", "SRR3726124")
dex = c("queen", "worker", "worker")
...
p
# ggsave(plot = p, filename = "/Volumes/Pop_Gen/Differential_gene_expression/DGE_volcano_plots/Camponotus_floridanus.pdf", width = 7, height = 7) #CHANGE NAME
...
DEGs <- dplyr::filter(topT, (abs(log2FoldChange) > 1), padj <0.05) #1289 genes


##Selecting differentially expressed genes based on the top 10% most positive and most negative values, and P-value<0.05 criteria
# Filter rows with padj < 0.05 and negative log2FoldChange
filtered_neg <- topT %>%
  filter(padj < 0.05, log2FoldChange < 0)
#Identify the 0.1th percentile
neg_thresh <- quantile(filtered_neg$log2FoldChange, 0.1)
# Select rows with values below the threshold
neg_10_percent <- filtered_neg[filtered_neg$log2FoldChange <= neg_thresh, ]

filtered_pos <- topT %>%
  filter(padj < 0.05, log2FoldChange > 0)
#Identify the 0.9th percentile
pos_thresh <- quantile(filtered_pos$log2FoldChange, 0.9)
# Select rows with values below the threshold
pos_10_percent <- filtered_pos[filtered_pos$log2FoldChange >= pos_thresh, ]

# Combine the results into a new dataframe
Strict_DEGs <- bind_rows(neg_10_percent, pos_10_percent)
...
Strict_DEGs$biased_gene <- ifelse(Strict_DEGs$log2FoldChange >= 0, "worker", "queen")

#Save as csv file
# write.csv(DEGs, "/Volumes/Pop_Gen/Differential_gene_expression/DGE_csv_outputs/Camponotus_floridanus.csv", row.names = T)
# write.csv(Strict_DEGs, "/Volumes/Pop_Gen/Differential_gene_expression/DGE_csv_outputs/Camponotus_floridanus_strict_DEG.csv", row.names = T)
